...
export default function Cart() {
  ...
  const discount =
    customerType === 'Member' && selectedMember
      ? (subtotal * memberDiscountPercent) / 100
      : 0;
  ...
  return (
    <div className={styles.cart}>
      ...
      <div className={styles.summary}>
        ...
        {discount > 0 && (
          <p className={styles.discount}>
            Diskon Member ({memberDiscountPercent}%): <span>- Rp {discount.toLocaleString('id-ID')}</span>
          </p>
        )}
        <hr />
        ...
        <div className={styles.memberSearch}>
          {selectedMember ? (
            ...
            <>
              ...
              <button onClick={() => setShowNewMemberForm(true)} style={{ marginTop: '10px' }}>
      ...
      {showNewMemberForm && (
        <div className={styles.modal}>
          <div className={`${styles.modalContent} card`}>
            ...
            <button onClick={() => setShowNewMemberForm(false)} className="button-secondary" style={{ marginTop: '10px' }}>
              Tutup
            </button>
          </div>
        </div>
      ...
      <div className={styles.actions}>
        <button onClick={handleCompleteTransaction} className={styles.checkoutBtn} disabled={cart.length === 0}>
